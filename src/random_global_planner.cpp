        ...
        if (v_pos_coll_tether.size() > 0){
            for (size_t i_ = 0 ; i_ < v_pos_coll_tether.size() ; i_++){
                if ((v_pos_coll_tether[i_] == j_)){
                    double l_;
                    if(getTetherLengthParams(trajectory_.points.at(i).transforms[0].translation, trajectory_.points.at(i).transforms[0].rotation, trajectory_.points.at(i).transforms[1].translation, l_)){
                        l_catenary_[j_] = l_;  
                    }
                }
            }           
			...
			for(int j=0 ; j < n_interval_ ; j++){
                ...
                if(is_ugv_free_){
                    ...
                    if (please_){
                        ...
                        ROS_ERROR("NOT, UGV NOT RANDOM SOLUTION !!");
                }
                ...
                if(j != 0){  
                    ...
                    double l_tether_;
                    bool is_cat_ = getTetherLengthParams(t_.transforms[0].translation, t_.transforms[0].rotation, t_.transforms[1].translation, l_tether_);             
...
bool RandomGlobalPlanner::getTetherLengthParams(geometry_msgs::Vector3 tp1_ , geometry_msgs::Quaternion tq1_, geometry_msgs::Vector3 tp2_, double &length_)
{

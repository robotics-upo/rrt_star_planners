#ifndef NEAREST_NODE_HPP
#define NEAREST_NODE_HPP
...
class NearNeighbor
{
	public:
	    ...
	    virtual void setInput(const std::vector<Eigen::Vector4d> &obs_);
		virtual void setInput(const sensor_msgs::PointCloud2 &pc2);

		virtual void setKDTree(const std::vector<Eigen::Vector3d> &obs_);
		virtual void setKDTree(const std::vector<Eigen::Vector4d> &obs_);
		...
		virtual std::vector<Eigen::Vector3d> radiusNearNodes(const pcl::KdTreeFLANN <pcl::PointXYZ> &kdT_, Eigen::Vector3d vert_, double radius_, pcl::PointCloud <pcl::PointXYZ>::Ptr o_p_);
...
inline void NearNeighbor::setInput(const std::vector<Eigen::Vector4d> &obs_){setKDTree(obs_);}
...
{
	ROS_INFO("NearNeighbor: Executed SetKDTree std::vector<Eigen::Vector3d>");
	...
	ROS_INFO("NearNeighbor: Size obs_points = [%lu]",obs_points->size());

	kdtree.setInputCloud(obs_points);
}

inline void NearNeighbor::setKDTree(const std::vector<Eigen::Vector4d> &obs_)
{
	ROS_INFO("NearNeighbor: Executed SetKDTree std::vector<Eigen::Vector4d>");
	obs_points.reset(new pcl::PointCloud <pcl::PointXYZ>);
...
{
	ROS_INFO("NearNeighbor: Executed SetKDTree PointCloud2");
	obs_points.reset(new pcl::PointCloud <pcl::PointXYZ>);
...
inline std::vector<Eigen::Vector3d> NearNeighbor::radiusNearNodes(const pcl::KdTreeFLANN <pcl::PointXYZ> &kdT_, Eigen::Vector3d vert_, double radius_, pcl::PointCloud <pcl::PointXYZ>::Ptr o_p_)
{  
	Eigen::Vector3d point_;
	std::vector<Eigen::Vector3d> v_points_;
	v_points_.clear();

	pcl::PointXYZ searchPoint(vert_.x(),vert_.y(),vert_.z());

	// Neighbors within radius search
	std::vector<int> pointIdxRadiusSearch_;
	std::vector<float> pointRadiusSquaredDistance_;
	
	// Get closest point in a radius
	kdT_.radiusSearch(searchPoint, radius_, pointIdxRadiusSearch_, pointRadiusSquaredDistance_);

	for (std::size_t i = 0; i < pointIdxRadiusSearch_.size (); ++i){
		point_.x() = o_p_->points[pointIdxRadiusSearch_[i]].x;
		point_.y() = o_p_->points[pointIdxRadiusSearch_[i]].y;
		point_.z() = o_p_->points[pointIdxRadiusSearch_[i]].z;

		v_points_.push_back(point_);
	}
